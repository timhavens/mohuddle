...
func TestClientImplementsScopedReconnectAndGapStates(t *testing.T) {
	...
	for _, value := range []string{
		...
		`challenge.payload`,
		`mode: "ask"`,
		`api.request("command.invoke", { command: "stop" }`,
		`text === "/stop"`,
		...
		`message-highlight`,
		`"routing.resolve"`,
		`Answered as chat — no work was implemented`,
		`Work request — implementation starts or queues`,
		`View answer`,
		`message-highlight`,
		`Replace and cancel the active workflow`,
