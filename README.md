...
AGY and Copilot default to `read-only`. While optional, their direct and invited turns are isolated and transcript-only; when promoted to active core, the same profile permits read-only inspection and normal core participation without mutation. Set either to `workspace` or `full` when direct or lead turns should perform coding work. Demotion restores the optional isolated behavior without deleting its native session. Core review, moderator closing, `/round`, and `/ask` turns remain read-only without changing any saved profile. For agents configured as `full`, these tasks retain full-machine filesystem reads, including neighboring repositories and symlink targets, while provider controls or an OS filesystem sandbox block writes. Read-only tasks retain their network restrictions. Explicitly tool-free routing and arbitration still receive no filesystem access. Agent activity and `/settings` show the effective task policy separately from the configured profile.
...
- AGY read-only **inspection** turns additionally wrap the entire provider process in a filesystem write sandbox: Linux/WSL requires `bwrap` (the `bubblewrap` package) and usable namespaces; macOS requires `sandbox-exec`. AGY's Plan mode alone does not block direct file edits. These inspection turns are ephemeral; native session/cache writes are denied and do not replace a saved worker session. Missing or unsupported containment fails with an explicit error. Windows AGY inspection is currently unavailable; use Codex, Claude, or Copilot for read-only inspection there. Transcript-only AGY replies do not require this wrapper.
...
The ordinary tests use fake CLIs and do not consume provider usage. `make live-test` sets `MOHUDDLE_LIVE=1`, invokes authenticated Codex and Claude, and may consume provider quota. To check a provider's full-machine read-only policy using disposable fixtures, run `MOHUDDLE_PERMISSION_PROVIDER=codex MOHUDDLE_PERMISSION_WRITE_PROBE=1 go test ./internal/integration -run '^TestLiveFullMachineReadOnly$' -count=1 -v` (replace `codex` with `claude`, `agy`, or `copilot`). This opt-in test reads files inside and outside its temporary workspace, attempts fixture writes, and verifies that both originals remain unchanged. It uses provider quota and never connects to a running MoHuddle room.
